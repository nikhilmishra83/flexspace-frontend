...
export default function Navbar() {
  ...
  const { user, logout } = useAuthStore(); // <-- Hook into store

  const navLinks = [
    { name: 'Find a Desk', href: '/user/spaces' },
    { name: 'My Bookings', href: '/user/dashboard' }, // Changed to dashboard to match your new UI
  ];
    ...
    <header className="border-b border-zinc-100 bg-white sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <Link href="/user/dashboard" className="flex items-center gap-2 text-black shrink-0">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3 border-l pl-6 border-zinc-100">
              ...
              <div className="text-left leading-tight hidden sm:block">
                <p className="text-xs font-black uppercase tracking-tighter text-zinc-900">{displayName}</p>
                <p className="text-[10px] text-blue-600 font-bold uppercase tracking-widest">Premium Member</p>
              </div>
            </div>
            {/* Replaced Link with Button to trigger logout function */}
            <button 
              onClick={logout} 
              className="text-[10px] font-black text-zinc-400 hover:text-red-500 uppercase tracking-widest transition-colors"
            >
              Sign Out
            </button>
          </div>
